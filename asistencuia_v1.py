        ...
        for horario in horarios:
            ...
            if info_dia_actual:
                ...
                if documento_existente:
                    ...
                    # Si hay una entrada válida y no se ha registrado la salida aún
                    if len(hec_actual) < 2 and not hsc_actual and estatus_entrada not in ["FUERA DE RANGO"]:
                    ...
                    # Si ya es hora de registrar la salida
                    if estatus_salida in ["NORMAL", "NO REGISTRADO"] and len(hsc_actual) < 2:
                        # Registrar la salida sin tocar la entrada
                        update_hsc = {"$push": {
                            "Fechas.$.HSC": {
                    ...
                    nuevo_subdocumento = {
                        ...
                        "HEC": [{"hora_entrada": hora_actual, "estatus_checador": estatus_entrada}] if estatus_entrada not in ["FUERA DE RANGO"] else [],
                        "HSC": [{"hora_salida": hora_actual, "estatus_checador": estatus_salida}] if estatus_salida in ["NORMAL", "NO REGISTRADO"] else []
                    }
